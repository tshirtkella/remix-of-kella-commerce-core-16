...
const ICON_MAP: Record<string, any> = { Shield, CreditCard, RotateCcw, Truck, Clock, Award };

const TrustBadges = () => {
  const content = usePageSection("home", "trust_badges");
  const badges = content?.badges ?? DEFAULT_BADGES;

  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {badges.map((b: any, i: number) => {
          const Icon = ICON_MAP[b.icon] || DEFAULT_BADGES[i]?.icon || Shield;
          return (
            <div key={b.label || i} className="flex flex-col items-center text-center gap-1.5 p-3 rounded-xl bg-card border border-border">
